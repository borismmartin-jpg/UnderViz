...
export function runPipeline(hours, site, depth) {
  ...
  for (const hr of hours) {
    ...
    // Wave components: two swell trains + wind sea.
    const comps = [];
    for (const [label, sw] of [['swell1', hr.swell1], ['swell2', hr.swell2]]) {
      const H = sw?.height ?? 0;
      const T = sw?.period ?? 0;
      comps.push({
        label, H, T, dir: sw?.direction ?? null,
    ...
    if (model?.height > 0.01 && model?.period > 0.5) {
      ws = model.height <= local.H
        ? { H: model.height, T: model.period, dir: model.direction ?? wdir, src: 'model' }
        : { H: local.H, T: local.T, dir: wdir, src: 'fetch-capped' };
    } else {
      ws = { H: local.H, T: local.T, dir: wdir, src: 'computed' };
    }
    comps.push({
      label: 'windsea', H: ws.H, T: ws.T, dir: ws.dir, src: ws.src,
      ub: bedOrbitalVelocity(ws.H, ws.T, d), power: wavePower(ws.H, ws.T),
    });
